...
fn main() {
    ...
    pause_ms(10); // Make sure Thread A has time to get going before we spawn Thread B
    ...
    // the child threads.
    println!("[Main] ---- starting my experiment ------");

    let (mytx, myrx) = channel::unbounded();

    let myrx1 = myrx.clone();
    let child1 = thread::spawn(move || {
        for msg in myrx1 {
            println!("[Child1] received: {}", msg);
        }
        println!("[Child1] done");
    });
    pause_ms(100);
    ...
    for m in 7..17 {
        println!("[Main] sending {}", m);
        mytx.send(m).unwrap();
    }

    pause_ms(500);
    println!("[Main] closing tx");
    drop(mytx);
    pause_ms(20);
    println!("[Main] stoping children");
    child1.join().unwrap();
    child2.join().unwrap();
